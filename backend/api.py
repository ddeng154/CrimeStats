...
def countySingle(result):
    ...
    for ori in oris:
        ...
        result['crimes'].extend([{'id': c.id, 'type': c.type, 'pd_name': name} for c in Crime.query.filter_by(ori=ori).all()])

def policeSingle(result):
    links = PoliceCountyLink.query.filter_by(ori=result['ori']).all()
    cids = {link.county_id for link in links}
    result['counties'] = [{'id': cid, 'name': County.query.get(cid).name} for cid in cids]
    result['crimes'] = [{'id': c.id, 'type': c.type} for c in Crime.query.filter_by(ori=result['ori']).all()]
