...
class ComparePoliceDepartments extends React.Component {
    ...
    handleSubmit() {
        ...
        if (this.state.id1 && this.state.id2) {
            const request1 = axios.get<PoliceDepartmentData>("/api/police_departments/" + this.state.id1);
            const request2 = axios.get<PoliceDepartmentData>("/api/police_departments/" + this.state.id2);
            axios.all<AxiosResponse<PoliceDepartmentData>>([request1, request2]).then(axios.spread((response1, response2) => {
                this.setState({
    ...
    render() {
        const Form = (<div>
            ...
            <Button onClick={this.handleSubmit}>Compare</Button></div>
        );
        if (this.state.isLoading) {
            return (
                <div className="text-center">
                    {Form}
                    Please enter 2 valid ORIs (look in the URLs of instance pages for ORIs)
                </div>
            );
        } else {
            return (
                    ...
                    <table className="table">
                        <thead>
                            <tr>
                                ...
                                <th>
                                    <a href={"/policedepartments/" + this.state.element1!.ori}>{this.state.element1!.name}</a>
                                </th>
                                <th>
                                    <a href={"/policedepartments/" + this.state.element2!.ori}>{this.state.element2!.name}</a>
                                </th>
                            </tr>
                        </thead>
                        <tbody>
